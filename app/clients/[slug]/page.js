...
export default function ClientDashboard() {
  ...
  // slug is URL-encoded client name, e.g. "Emile%20Sakhel"
  ...
  // Guard: only admin and staff can view this page
  useEffect(() => {
    if (status === 'loading') return;
    if (status === 'unauthenticated') { router.replace('/login'); return; }
    const role = session?.user?.role;
    if (role !== 'admin' && role !== 'staff') { router.replace('/'); return; }
  }, [status, session, router]);

  // Helper: build fetch URL with ?client= injected for admin override
  const apiUrl = (path, params = {}) => {
    const p = new URLSearchParams({ ...params, client: clientName });
  ...
  useEffect(() => {
    const role = session?.user?.role;
    if ((role !== 'admin' && role !== 'staff') || !clientName) return;
  ...
  // Re-fetch listing table when month or listing changes
  useEffect(() => {
    const role = session?.user?.role;
    if (!selectedMonth || (role !== 'admin' && role !== 'staff')) return;
    async function fetchListings() {
      try {
        const params = { month: selectedMonth };
        if (selectedListing && selectedListing !== 'all') params.listing = selectedListing;
        const res  = await fetch(apiUrl('/api/listings', params));
    ...
    fetchListings();
  }, [selectedMonth, selectedListing, session, clientName]);

  // Fetch all months for a selected listing
  useEffect(() => {
    if (selectedListing === 'all') { setListingTrends([]); return; }
    const role = session?.user?.role;
    if (role !== 'admin' && role !== 'staff') return;
    async function fetchListingTrends() {
      try {
        const res  = await fetch(apiUrl('/api/listings', { listing: selectedListing }));
        const json = await res.json();
    ...
    fetchListingTrends();
  }, [selectedListing, session, clientName]);
  ...
  if (status === 'loading' || (status === 'authenticated' && _role !== 'admin' && _role !== 'staff')) {
    return (
        ...
        <div className="max-w-screen-2xl mx-auto flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          <div>
            ...
            <div className="flex items-center gap-3 mb-2">
              <Image src="/logo.png" alt="Pricing By Mira" width={100} height={67} className="rounded" style={{ objectFit: 'contain' }} priority />
              <button
                onClick={() => router.push('/clients')}
                className="text-amber-500 text-xs font-semibold tracking-widest hover:text-amber-400 flex items-center gap-1"
              >
                ← ALL CLIENTS
              </button>
            </div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white">{clientName}</h1>
            <p className="text-slate-400 text-sm mt-1">Client Performance Dashboard</p>
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 bg-[#2a2c30] border border-[#383b40] rounded-full px-3 py-1">
              {session?.user?.role === 'admin' ? '🔐 Admin View' : '👁 Staff View'}
            </span>
            <button
              onClick={() => window.print()}
              className="no-print px-4 py-2 bg-[#2a2c30] border border-[#383b40] rounded-lg text-slate-300 text-sm hover:bg-[#32353a] transition-colors"
            >
              Export PDF
            </button>
          </div>
